...
- **Optional bulk leaf index (non-normative):** publishers may upload an additional ANS-104 JSON artifact tagged `App=vincent`, `Epoch`, and `Kind=leaf-uris` containing `leafKey → ar://txId` hints bound to the epoch fingerprint. Clients may use it for faster gateway-first decode; normative verification still uses per-leaf tag query or Merkle proof checks.

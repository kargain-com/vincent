...
function parseArgs(argv: string[]): CliOptions {
  ...
  return { devnet, fixture, verifyOnly, publisher, manifestUri };
}
...
async function runVerifyOnly(options: CliOptions): Promise<void> {
  if (options.publisher === undefined || options.manifestUri === undefined) {
    throw new Error(
      'Usage: publish-genesis --devnet --verify-only --publisher <address> --manifest-uri ar://...',
    );
  }
...
async function runPublish(options: CliOptions): Promise<void> {
  ...
  process.stdout.write('Running preflight (registry, RPC, balance, Irys)...\n');
  ...
  process.stdout.write('Publishing genesis epoch...\n');
...
async function main(): Promise<void> {
  ...
  if (!options.devnet) {
    throw new Error(
      'Usage: publish-genesis --devnet [--fixture genesis-mini|full] | --verify-only --publisher <addr> --manifest-uri ar://...',
    );
  }
